<?php
	...
	if($stmt = $conn->prepare($sql))
	{
		...
		if($stmt->num_rows!=0)
		{
			?><h1>Publications of year <?php echo $yr; ?> with Research area <?php echo $ra; ?>.</h1><?php
			...
			while ($stmt->fetch())
			{
					...
					$sql2 = "select * from research where r_id in (select r_id from publication_ra where pub_id=?);";
